/proc/browser_input_list(mob/user, message, title = "", list/items, default, timeout = 0)
	if(!user)
		user = usr
	if(!length(items))
		return null
...
/proc/input_bitfield(mob/User, title, bitfield, current_value, width = 350, width = 350, nslidecolor, allowed_edit_list = null)
	...
	var/list/result = presentpicker(User, "", title, Button1="Save", Button2 = "Cancel", Timeout=FALSE, values = pickerlist, width = width, height = width, slidecolor = nslidecolor)
	if (islist(result))
