...
/obj/structure/fluff/walldeco/psybanner/get_mechanics_examine(mob/user)
	...
	. += span_info("The PANTHEON, as the Church's official religion, is considered the 'status quo' within Lyndvhar. They worship ONE of TEN GODS, all embodying certain aspects and virtues of humenity.")
	. += span_info("The ASCENDANTS are considered 'heathens' by the PANTHEON, and are actively hunted if made apparent within Lyndvhar. They worship ONE of FOUR MORTALS-TURNED-GODS, who seek to usurp the 'status quo'.")
...
/obj/structure/fluff/walldeco/med4/get_mechanics_examine(mob/user)
	...
	. += span_info("Those who've died beyond Lyndvhar's walls will eventually become deadites; the lyving dead. Shambling back to the Town might offer them a second chance at lyfe, if they aren't laid to rest first.")
